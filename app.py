...
def fetch_and_cache_movies():
    ...
    print("[CACHE] Fetching Tamil OTT movies...")
    ...
    for page in range(1, 1000):
        ...
        params = {
            ...
            "with_original_language": "ta",  # changed from "ml" (Malayalam) to "ta" (Tamil)
    ...
    print(f"[CACHE] Fetched {len(all_movies_cache)} Tamil OTT movies ✅")
...
def manifest():
    return jsonify({
        "id": "org.tamil.catalog",
        "version": "1.0.0",
        "name": "Tamil",
        "description": "Latest Tamil Movies on OTT",
        "resources": ["catalog"],
        "types": ["movie"],
        "catalogs": [{
            "type": "movie",
...
@app.route("/catalog/movie/tamil.json")
def catalog():
